...
int main(int argc, char *argv[])
{
    ...
    test = "    This is the OpenFOAM tutorial.  ";
    ...
    */
    ...
    /* Info << string(test).replace(" ","n") << endl; // Only removes the first space character.
    Info << string(test).replace(" ", "") << endl; //One way of removing spaces
    Info << string(test).replaceAll("i","n") << endl; //Remove all the characters
    Info << string(test).replaceAll(" ", "") << endl; // The way of removing all the spaces
    Info << string(test).replaceAll("the","") << endl; */ 
    
    /*
    string test2 = "This tutorial we are running in $FOAM_RUN . The user is from $HOME .";
    Info<<test2<<endl;
    Info<<test2.expand()<<endl; // Extends variables $FOAM_RUN and $HOME. 
    */ 
   ...
   string s2(s.expand()); 
    ...
    cout << "ostream<< >" << s2 << "<\n";
    cout << "Ostream<< >" << s2 << "<\n";
    Info << "hash: " << hex << string::hash()(s2) << endl; // Hash is more cost-effective in comparing very large strings.
    Info << "End\n" << endl;
    // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
